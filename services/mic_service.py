...
El mixin asume que la clase que lo hereda tiene:
    ...
    - self.transcription_service.pipeline: AudioPipeline para procesamiento
...
class MicTestMixin:
    ...
    def _mic_test_worker_inner(self):
        ...
        try:
            ...
            proc = self.transcription_service.pipeline.process(raw)
    ...
    def _compute_mic_metrics(self, raw, proc):
        ...
        w = int(0.04 * SR)
        hop = w // 2
        fr_r = self.transcription_service.pipeline._frame_rms(raw.astype(np.float64), w, hop)
        fr_p = self.transcription_service.pipeline._frame_rms(proc.astype(np.float64), w, hop)
        ...
        lines.append(f"Pico: {pk:.3f} (limite {self.transcription_service.pipeline.p['limiter']:.2f}, sin clipping)")
        return "\n".join(lines)
...
# Alias de servicio (nombre canónico de la extracción).
MicService = MicTestMixin
