...
def _trim_silence(
    audio: np.ndarray, sr: int, threshold: float = RMS_SILENCE, min_silence_sec: float = 0.5
) -> np.ndarray:
...
def solve_audio_issues(
    audio: np.ndarray, sr: int = 16000, report: AudioQualityReport | None = None, config: dict | None = None
) -> tuple[list[SoundFix], np.ndarray]:
