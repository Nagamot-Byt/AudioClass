...
check("debil abre dialogo (no graba aun)", s.opened_warn == 1 and s.began == 0, f"warn={s.opened_warn} began={s.began}")
...
check(
    "open_opt lanza Aplicar optimizacion",
    s.opt_started == 1 and s.opt_apply is True,
...
def _run_live(level, fail=False):
    ...
    ac.sd.InputStream = lambda **kw: FakeStream(np.full(int(0.1 * ac.SAMPLE_RATE), level, np.float32), fail=fail, **kw)
...
msgs = _run_live(0.05)
check("live worker envia nivel alto", any(mt == "mic_live" and r > 0.04 for mt, r in msgs), str(msgs[:2]))
msgs = _run_live(0.001)
check("live worker envia nivel bajo", any(mt == "mic_live" and r < 0.002 for mt, r in msgs), str(msgs[:2]))
...
check(
    "best sube a verde tras hablar",
    s.mic_warn_best_p90 >= 0.03 and "Meta alcanzada" in s.mic_warn_best_lbl.text,
    f"best={s.mic_warn_best_p90}",
)
for _ in range(30):  # la voz baja: el max NO debe caer
    ac.App._update_mic_warn_best(s, 0.001)
check("best no baja aunque la voz baje", s.mic_warn_best_p90 >= 0.03, f"best={s.mic_warn_best_p90}")
...
check(
    "best inicial debil = warn con meta",
    s2.mic_warn_best_p90 < 0.03 and "meta 0.03" in s2.mic_warn_best_lbl.text,
    f"best={s2.mic_warn_best_p90}",
)
...
check(
    "trend acumula ventanas p90",
    len(getattr(s3, "mic_warn_p90_hist", [])) >= 2,
    str(len(getattr(s3, "mic_warn_p90_hist", []))),
)
check("trend valores reflejan la senal", all(v >= 0.03 for v in s3.mic_warn_p90_hist), str(s3.mic_warn_p90_hist))
for _ in range(200):  # muchas mas -> cap a 20 barras
    ac.App._update_mic_warn_best(s3, 0.05)
check("trend limitado a 20 ventanas (~10 s)", len(s3.mic_warn_p90_hist) <= 20, str(len(s3.mic_warn_p90_hist)))
...
check("probe worker envia nivel alto", mt == "mic_probe" and lvl is not None and lvl >= 0.09, f"mt={mt} lvl={lvl}")
mt, lvl = _run_probe(1e-9)
check(
    "probe worker envia nivel ~silencio",
    mt == "mic_probe" and lvl is not None and lvl < ac.MIC_PROBE_P90_MIN,
    f"mt={mt} lvl={lvl}",
)
