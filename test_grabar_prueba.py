...
with tempfile.TemporaryDirectory() as tmp:
    ...
    try:
        ...
        m.OUTPUT_DIR = old
    check(
        "process_and_save: limiter del perfil devuelto",
        isinstance(limiter, float) and 0 < limiter <= 1.0,
        f"= {limiter:.3f}",
    )
    ...
    check(
        "pipeline: VAD recorta (out <= in, no vacio)",
