...
check("silencio detectado como FAIL", r_silence.verdict == "FAIL", f"verdict={r_silence.verdict}")
check("silencio tiene issue 'silence'", "silence" in r_silence.issues, f"issues={r_silence.issues}")
check("silencio NO es auto_fixable", not r_silence.auto_fixable, f"auto_fixable={r_silence.auto_fixable}")
check("silencio severity es error", r_silence.severity == "error", f"severity={r_silence.severity}")
...
check("solver sin correcciones para silencio", len(fixes_silence) == 0, f"fixes={len(fixes_silence)}")

# Verificar acciones manuales
actions_silence = suggest_manual_actions(r_silence)
check("acciones manuales para silencio mencionan Windows", any("Windows" in a for a in actions_silence))
...
check("WAV silencio -> FAIL (gate bloquea)", r_wav_sil.verdict == "FAIL", f"verdict={r_wav_sil.verdict}")
check(
    "WAV silencio message contiene 'insuficiente'",
...
check("audio debil detectado como WARN o FAIL", r_weak.verdict in ("WARN", "FAIL"), f"verdict={r_weak.verdict}")
check("audio debil tiene issues", len(r_weak.issues) > 0, f"issues={r_weak.issues}")
check("audio debil es auto_fixable", r_weak.auto_fixable, f"auto_fixable={r_weak.auto_fixable}")
...
check("solver aplica correcciones a audio debil", len(fixes_weak) > 0, f"fixes={len(fixes_weak)}")
...
check("nivel sube despues del fix", rms_after > rms_before, f"before={rms_before:.6f}, after={rms_after:.6f}")

# Verificar acciones manuales
actions_weak = suggest_manual_actions(r_weak)
check(
    "acciones manuales para debil mencionan ganancia",
...
check(
    "WAV debil -> WARN o FAIL (gate advierte)", r_wav_weak.verdict in ("WARN", "FAIL"), f"verdict={r_wav_weak.verdict}"
)
...
audio_clip = np.clip((0.2 * np.sin(2 * np.pi * 300 * t) + 0.1 * np.sin(2 * np.pi * 800 * t)) * 5.0, -1.0, 1.0).astype(
    np.float32
)
r_clip = check_audio_quality(audio_clip, sr=SR)

check("clipping detectado", r_clip.clipping_pct > 0, f"clipping={r_clip.clipping_pct}")
check("clipping tiene issue", "clipping" in r_clip.issues, f"issues={r_clip.issues}")

# Solver debe normalizar clipping
fixes_clip, fixed_clip = solve_audio_issues(audio_clip, sr=SR, report=r_clip)
check("solver corrige clipping", len(fixes_clip) > 0, f"fixes={len(fixes_clip)}")
check("pico reducido post-fix", float(np.max(np.abs(fixed_clip))) <= 1.0, f"peak={np.max(np.abs(fixed_clip))}")

# Verificar que el fix reporta bien
fix_text = format_fix_report(fixes_clip)
check(
    "fix report contiene normalizacion",
    "Normalizado" in fix_text or "normaliz" in fix_text.lower(),
    f"text={fix_text[:100]}",
)
...
check("audio normal no es FAIL", r_normal.verdict != "FAIL", f"verdict={r_normal.verdict}")
check("audio normal p90 > 0.05", r_normal.rms_p90 > 0.05, f"p90={r_normal.rms_p90}")
...
check(
    "WAV normal -> OK o WARN (gate permite)", r_wav_normal.verdict in ("OK", "WARN"), f"verdict={r_wav_normal.verdict}"
)
...
check("procsave: calidad detectada", r_to_fix.verdict != "OK", f"verdict={r_to_fix.verdict}")

# Paso 2: solver aplica fixes
fixes, fixed = solve_audio_issues(audio_to_fix, sr=SR, report=r_to_fix)
check("procsave: fixes aplicados", len(fixes) > 0, f"fixes={len(fixes)}")
...
check("procsave: nivel mejorado", rms_fixed > rms_orig, f"orig={rms_orig:.6f}, fixed={rms_fixed:.6f}")

# Paso 4: re-check post-fix mejora
r_post = check_audio_quality(fixed, sr=SR)
check(
    "procsave: post-fix mejor veredicto",
...
check("reporte FAIL contiene '[FAIL]'", "[FAIL]" in txt_fail, f"txt={txt_fail[:80]}")
check("reporte FAIL contiene 'p90'", "p90" in txt_fail)

txt_warn = format_report_text(r_weak)
check("reporte WARN contiene '[WARN]'", "[WARN]" in txt_warn, f"txt={txt_warn[:80]}")

txt_ok = format_report_text(r_normal)
check("reporte OK contiene '[OK]' o '[WARN]'", "[OK]" in txt_ok or "[WARN]" in txt_ok)
