...
try:
    from audioclass_v91 import PALETTES, C, GeminiAdaptationEngine, _btn_text_color, _gdocs_importable
except ImportError:
    C = {}
    PALETTES = {}
...
def build_sidebar(app: App):
    ...
    sb.grid(row=0, column=0, rowspan=2, sticky="nsew", padx=(15, 8), pady=8 if getattr(app, "_compact", False) else 15)
    sb.grid_propagate(False)

    app._lbl(sb, " Historial de Clases", font=(app.FH, 16, "bold"), text_color=C["text"]).pack(
        pady=(18, 12), padx=15, anchor="w"
    )
    ...
    try:
        ...
        app.bplay = app._btn(bf, "Reproducir", app._play, state="disabled", width=260, height=32, fg_color=C["accent"])
        app.bplay.pack(fill="x", pady=(0, 6))

    bf = app._frame(sb, fg_color="transparent")
    bf.pack(fill="x", padx=10, pady=(0, 15))

    app.btransh = app._btn(bf, "Transcribir", app._transh, state="disabled", width=260, height=32)
    app.btransh.pack(fill="x", pady=(0, 6))
    app.bdel = app._btn(
        bf, "Eliminar", app._delh, state="disabled", width=260, height=32, fg_color=C["err"], hover_color=C["err"]
    )
    app.bdel.pack(fill="x", pady=(0, 6))
    app.bcompile = app._btn(
        bf,
        "Compilar Todo",
        app._compile,
        state="disabled",
    ...
    app.bguide = app._btn(
        bf, "Guia Rapida", app._open_guide, width=260, height=32, fg_color=C["accent"], hover_color=C["accent_hover"]
    )
...
def build_header(app: App, parent):
    ...
    hd = app._frame(parent, fg_color=C["header"], border_width=1, border_color=C["border"], theme_key="header")
    ...
    app._lbl(brand, "AC", font=(app.FH, 24), text_color=C["head_text"], theme_key="head_text").pack(
        side="left", padx=(0, 12)
    )
    ...
    app._lbl(
        btb,
        app.app_name if hasattr(app, "app_name") else "AudioClass",
        font=(app.FH, 21, "bold"),
        text_color=C["head_text"],
        theme_key="head_text",
    ).pack(anchor="w")
    app._lbl(
        btb,
        "Grabacion y transcripcion academica con IA",
        font=(app.FB, 11),
        text_color=C["head_text"],
        theme_key="head_text",
    ).pack(anchor="w")
    ...
    app.lconn = app._lbl(hdr, "Motor local", font=(app.FB, 11), text_color=C["head_text"], theme_key="head_text")
    app.lconn.pack(side="left", padx=(0, 12))
    app.btheme_hd = app._btn(
        hdr, "Tema", app._theme, width=44, height=34, font=(app.FB, 14), fg_color=C["card"], hover_color=C["border"]
    )
    app.btheme_hd.pack(side="left")

    # Linea dorada
    app.goldline = (
        ctk.CTkFrame(hd, height=3, corner_radius=2, fg_color=C["accent"])
        if CTK
        else tk.Frame(hd, height=3, bg=C["accent"])
    )
    ...
    for i, (num, txt) in enumerate([("1", "Graba"), ("2", "Transcribe"), ("3", "Analiza"), ("4", "Guarda")]):
        ...
        if CTK:
            ...
            lbl = tk.Label(steps, text=f"  {num}. {txt}  ", font=(app.FB, 12, "bold"), bg=C["button"], fg=C["muted"])
    ...
    if not getattr(app, "_compact", False):
        nx = app._frame(hd, fg_color=C["card"], border_width=2, border_color=C["accent"])
        nx.grid(row=4, column=0, columnspan=2, sticky="ew", padx=18, pady=(8, 2))
        nx.grid_columnconfigure(0, weight=1)
        app.next_step_frame = nx
        app.lnext = app._lbl(nx, "", font=(app.FH, 15, "bold"), text_color=C["accent"], anchor="w", wraplength=1000)
        app.lnext.grid(row=0, column=0, sticky="ew", padx=(16, 8), pady=(6, 2))
        app.lnext_sub = app._lbl(nx, "", font=(app.FB, 11), text_color=C["muted"], anchor="w", wraplength=1000)
...
def build_easy_mode(app: App, parent):
    ...
    app._lbl(easy, "MODO FACIL", font=(app.FH, 16, "bold"), text_color=C["easy"]).pack(anchor="w", padx=18, pady=(2, 2))
    if not getattr(app, "_compact", False):
        app._lbl(
            easy,
            "Un solo boton hace TODO: Grabar -> Procesar -> Transcribir -> Analizar",
            font=(app.FB, 11),
            text_color=C["muted"],
    ...
    if CTK:
        app.easy_template = ctk.StringVar(value=app.config.get("adaptacion_default", "Analisis Academico Profundo"))
        app.easy_menu = ctk.CTkOptionMenu(
            easy_row, values=templates_list, variable=app.easy_template, width=260, font=(app.FB, 11)
        )
        app.easy_menu.pack(side="left", padx=(0, 10))
    else:
        app.easy_template = tk.StringVar(value=app.config.get("adaptacion_default", "Analisis Academico Profundo"))
        app.easy_menu = tk.OptionMenu(easy_row, app.easy_template, *templates_list)
        app.easy_menu.pack(side="left", padx=(0, 10))

    app._lbl(easy_row, "Selecciona que generar automaticamente", font=(app.FB, 10), text_color=C["muted"]).pack(
        side="left"
    )
...
def build_controls(app: App, parent):
    ...
    app.btr = app._btn(ct, "Transcribir", lambda: app._starttrans(False), width=150, height=42, state="disabled")
    app.btr.pack(side="left", padx=(0, 8), pady=10)
    app.bts = app._btn(ct, "Con tiempos", lambda: app._starttrans(True), width=130, height=42, state="disabled")
    app.bts.pack(side="left", padx=(0, 8), pady=10)
    app.bpdf = app._btn(ct, "Guardar PDF", app._pdf, width=130, height=42, state="disabled")
    app.bpdf.pack(side="left", padx=(0, 8), pady=10)
    app.bdocx = app._btn(ct, "Guardar DOCX", app._export_docx, width=140, height=42, state="disabled")
    ...
    app.bcancel = app._btn(
        ct, "Cancelar", app._cancel, width=100, height=42, state="disabled", fg_color=C["err"], hover_color=C["err"]
    )
    ...
    # Estado
    app.lstatus = app._lbl(ct, "Listo para grabar", font=(app.FB, 12), text_color=C["muted"])
...
def build_vu_meter(app: App, parent):
    ...
    if CTK:
        app.vu_bar = ctk.CTkProgressBar(
            vu_row1, width=170, height=10, corner_radius=5, fg_color=C["button"], progress_color=C["accent"]
        )
        app.vu_bar.set(0)
        app._gold_bars.append(app.vu_bar)
    else:
        app.vu_bar = tk.ttk.Progressbar(vu_row1, mode="determinate", length=150, maximum=100)
        app.vu_bar["value"] = 0
    app.vu_bar.pack(side="left", padx=(0, 8))
    app.vu_lbl = app._lbl(vu_row1, "-inf dB", font=(app.FB, 10), text_color=C["muted"])
    ...
    app.vu_hist = tk.Canvas(
        vu_row2, width=160, height=24, bg=C["card"], highlightthickness=1, highlightbackground=C["border"]
    )
    app.vu_hist.pack(side="left", padx=(0, 6))
    app._lbl(vu_row2, "Sens:", font=(app.FB, 9), text_color=C["muted"]).pack(side="left", padx=(0, 4))
    if CTK:
        app.vu_sens_slider = ctk.CTkSlider(
            vu_row2,
            from_=0.05,
            to=0.60,
            number_of_steps=11,
        ...
        app.vu_sens_slider = tk.ttk.Scale(
            vu_row2, from_=0.05, to=0.60, orient="horizontal", command=app._vu_sens_changed, length=110
        )
    app.vu_sens_slider.set(app.vu_sens)
    app.vu_sens_slider.pack(side="left", padx=(0, 6))
    app.vu_sens_val = app._lbl(vu_row2, f"{app.vu_sens:.2f}", font=(app.FB, 9), text_color=C["muted"])
...
def build_config_bar(app: App, parent):
    ...
    if CTK:
        app.cmb_profile = ctk.CTkOptionMenu(
            cfg,
            values=list(AudioPipeline.PROFILES.keys()),
        ...
        app.cmb_profile = tk.OptionMenu(cfg, app.profile_var, *AudioPipeline.PROFILES.keys(), command=app._chprofile)
    ...
    if CTK:
        ...
        tk.OptionMenu(cfg, app.mode_var, "local", "cloud", command=app._chmode).pack(side="left", padx=(0, 20), pady=12)
    ...
    if CTK:
        ...
        app.cmb_model = tk.OptionMenu(cfg, app.model_var, "tiny", "base", "small", command=app._chlocalmodel)
    ...
    if CTK:
        app.cmb_lang = ctk.CTkOptionMenu(
            cfg, values=_langs, variable=app.lang_var, width=82, command=app._chlang, font=(app.FB, 11)
        )
    else:
    ...
    if CTK:
        ...
        tk.Checkbutton(cfg, text="VAD", variable=app.vad_var, bg=C["card"], fg=C["text"]).pack(
            side="left", padx=(0, 15), pady=12
        )
...
def build_progress(app: App, parent):
    ...
    if CTK:
        app.pbar = ctk.CTkProgressBar(pr, height=12, corner_radius=6, progress_color=C["accent"], fg_color=C["button"])
        app.pbar.set(0)
...
def build_waveform(app: App, parent):
    ...
    if MPL:
        app.fig = Figure(figsize=(8, 0.6 if getattr(app, "_compact", False) else 1.5), dpi=100, facecolor=C["card"])
        ...
        app._lbl(vz, "Instala matplotlib: pip install matplotlib", text_color=C["warn"]).pack(pady=35)
...
def build_adapt(app: App, parent):
    ...
    adapt = app._frame(parent, fg_color=C["card"], border_width=1, border_color=C["gemini"])
    ...
    app._lbl(ah, "Adaptacion Inteligente", font=(app.FH, 14, "bold"), text_color=C["gemini"]).pack(side="left")
    app.ladapt = app._lbl(ah, "Sin API Key", font=(app.FB, 11), text_color=C["warn"])
    app.ladapt.pack(side="right")

    app._lbl(
        adapt, "Selecciona que quieres generar a partir de la transcripcion:", font=(app.FB, 11), text_color=C["muted"]
    ...
    if CTK:
        ...
        app.adapt_txt = scrolledtext.ScrolledText(
            adapt, wrap=tk.WORD, font=("Consolas", 11), bg=C["bg"], fg=C["text"], height=7
        )
    app.adapt_txt.grid(row=4, column=0, sticky="nsew", padx=18, pady=(0, 14))
    app.adapt_txt.configure(state="disabled")

    app.bsave_adapt = app._btn(
        adapt, "Guardar Adaptacion", app._save_adaptation, width=180, height=36, state="disabled"
    )
...
def build_transcription(app: App, parent):
    ...
    tr = app._frame(parent, fg_color=C["card"], border_width=1, border_color=C["border"])
    ...
    app._lbl(th, "Transcripcion", font=(app.FH, 15, "bold"), text_color=C["text"]).pack(side="left")
    app.lbadge = app._lbl(th, "Revisado por IA", font=(app.FB, 10, "bold"), text_color=C["ok"])
    ...
    app.lmodel = app._lbl(th, "Cargando...", font=(app.FB, 11), text_color=C["warn"])
    app.lmodel.pack(side="right")

    tbox = app._frame(tr, fg_color=C["bg"], border_width=1, border_color=C["border"])
    ...
    app.vsb = tk.Scrollbar(
        tbox, orient="vertical", command=app.txt.yview, bg=C["border"], troughcolor=C["bg"], relief="flat"
    )
...
def build_footer(app: App, parent):
    ...
    ft = app._frame(parent, fg_color=C["card"], border_width=1, border_color=C["border"])
    ft.grid(row=8, column=0, sticky="ew", padx=22, pady=(0, 5))
    ftl = app._frame(ft, fg_color="transparent")
    ftl.pack(side="left")
    app._lbl(ftl, f"{OUTPUT_DIR}", font=(app.FB, 10), text_color=C["muted"]).pack(side="left")
    app._lbl(
        ft,
        "Espacio  -  Ctrl+R grabar  -  Ctrl+S guardar  -  Ctrl+E exportar  -  F1 ayuda",
        font=(app.FB, 10),
        text_color=C["muted"],
    ).pack(side="right", padx=(0, 18))
    app._btn(ftl, "Abrir carpeta", app._open_output_dir, width=130, height=28, font=(app.FB, 10)).pack(
        side="left", padx=(12, 0)
    )
    app._btn(
        ftl,
