...
class ConfigDialogMixin:
    ...
    def open_config_dialog(self):
        ...
        from audioclass_core import CloudColabEngine as _CCE
        from audioclass_core import GoogleDocsExporter as _GDE
        ...
        # ── Cuerpo DESPLAZABLE ──────────────────────────────────────────────
        if CTK:
            body = ctk.CTkScrollableFrame(top, fg_color=C["bg"], corner_radius=0, scrollbar_button_color=C["border"])
        ...
        self._lbl(f1, "Proveedor de IA para el análisis", font=(self.FH, 13, "bold")).pack(
        ...
        self._lbl(
            f1g, "Consiguela gratis en: aistudio.google.com/app/apikey", font=(self.FB, 10), text_color=C["muted"]
        ...
        if CTK:
            for val, lbl in (("flash", "Flash"), ("pro", "Pro")):
                rb = ctk.CTkRadioButton(
                    gmod_row, text=lbl, variable=gemini_model, value=val, font=(self.FB, 11), text_color=C["text"]
                )
        ...
        self._lbl(
            f1g,
            "flash = rapido y economico (Gemini 2.0 Flash) | pro = maxima calidad (Gemini 2.5 Pro)",
            font=(self.FB, 10),
            text_color=C["muted"],
        ).pack(anchor="w", pady=(0, 4))
        ...
        self._lbl(
            f1o,
            "Consiguela en: platform.openai.com/api-keys (tiene plan gratuito inicial)",
            font=(self.FB, 10),
            text_color=C["muted"],
        ).pack(anchor="w", pady=(0, 4))
        ...
        for val, lbl in (("mini", "GPT-4o mini"), ("gpt4o", "GPT-4o")):
            rb = ctk.CTkRadioButton(
                omod_row, text=lbl, variable=openai_model, value=val, font=(self.FB, 11), text_color=C["text"]
            )
            rb.pack(side="left", padx=(0, 20))
            self._themeable.append(("label", rb, "text"))
        self._lbl(
            f1o, "mini = rapido y economico | GPT-4o = maxima calidad", font=(self.FB, 10), text_color=C["muted"]
        ).pack(anchor="w", pady=(0, 4))
        ...
        self._lbl(f2, "URL de ngrok desde tu servidor de Colab:", font=(self.FB, 10), text_color=C["muted"]).pack(
                ...
                if best_id is not None:
                    ...
                    if best_id < len(devs):
                        ...
                        self._mic_search_lbl.configure(
                            text=f"Encontrado: {found_name[:30]} (p90={best_p90:.4f})", text_color=C["ok"]
                        )
                    else:
        ...
        if CTK:
            ...
            gain_slider.pack(side="left", padx=(0, 8))
        self._lbl(
            gain_row, "(si tu microfono es muy subido, sube la ganancia)", font=(self.FB, 9), text_color=C["muted"]
        ).pack(side="left")
        ...
        self._lbl(f4, "Google Docs (exportar transcripciones)", font=(self.FH, 13, "bold")).pack(
            anchor="w", padx=15, pady=(12, 4)
        )
        self._lbl(
            f4,
            "1. Crea credenciales OAuth en console.cloud.google.com (tipo 'App de escritorio') y habilita la Docs API",
            font=(self.FB, 10),
            text_color=C["muted"],
        ).pack(anchor="w", padx=15, pady=(0, 2))
        self._lbl(
            f4, "2. Descarga el client_secret.json y seleccionalo:", font=(self.FB, 10), text_color=C["muted"]
        ...
        def _pick_creds():
            fp = filedialog.askopenfilename(
                title="Selecciona client_secret.json", filetypes=[("Credenciales JSON", "*.json")]
            )
            if fp:
        ...
        if not _gdocs_importable():
            try:
                ...
                self.gdoc_lbl.configure(
                    text="No disponible en esta versión: falta google-auth-oauthlib (no incluido en el instalador).",
                    text_color=C["warn"],
                )
            except Exception:
                pass
            ...
            try:
                ...
                self.gdoc_lbl.configure(
                    text=("[OK] " if gok else "· ") + gmsg, text_color=C["ok"] if gok else C["muted"]
                )
            except Exception:
                pass
        ...
        if CTK:
            ctk.CTkCheckBox(
                fp,
                text="Permito el análisis con IA (envío de mis transcripciones a Gemini/OpenAI)",
                variable=ia_consent_var,
                font=(self.FB, 11),
                fg_color=C["accent"],
        ...
        def save():
            ...
            self.docs_exporter = _GDE(self.config["google_creds_path"])
        ...
        self._btn(bar, "Guardar Cambios", save, width=200, height=40, fg_color=C["accent"]).pack(pady=12)
