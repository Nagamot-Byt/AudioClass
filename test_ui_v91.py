...
def synth_speech(amp=1.0):
    ...
    while pos < end:
        ...
        seg = (
            burst(rng.uniform(250, 900), 0.25 * amp, n)
...
check(
    "_mic_metrics: voz detectada (SNR positivo)",
    "[OK] Voz detectada" in metrics and "SNR" in metrics,
    metrics.splitlines()[0],
)
check("_mic_metrics: menciona noise gate y limiter", "noise gate" in metrics and "limite" in metrics)
for line in metrics.splitlines():
    check(f"_mic_metrics linea finita: {line[:40]}", "nan" not in line.lower() and "inf" not in line.lower())
...
check(
    "_mic_metrics: solo-ruido -> aviso sin crashear",
    "[!]" in metrics_n or "Voz muy baja" in metrics_n,
    metrics_n.splitlines()[0],
)

# Audio vacio -> sin crashear
metrics_e = m.App._mic_metrics(fake, np.zeros(0, np.float32), np.zeros(0, np.float32))
check("_mic_metrics: audio vacio sin crashear", isinstance(metrics_e, str) and len(metrics_e) > 0, repr(metrics_e))
